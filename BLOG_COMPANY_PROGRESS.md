...
| **3** | テンプレート化 (複数サイト雛形) | 12-20h | ⏳ **3.1+3.2+3.4 完了 (2026-05-11)・残 3.3 のみ (実サイト要望待ち)** |
...
### ✅ Phase 3.2 完了 (2026-05-11): CLAUDE.md 逆適用

`fukuoka-golf-guide/CLAUDE.md` (221 行) 配置完了:
- テンプレ CLAUDE.md (157 行・全 `[REPLACE]` プレースホルダ) を実値で全埋め
- fukuoka 固有セクション 4 つ追加 (§13 5 エリア × 35 コース構造 / §14 Phase 履歴サマリ / 多言語 Tier 分類)
- 11 commit (2026-04-29 ~ 2026-05-10) の Phase 履歴を CLAUDE.md §14 にサマリ
- アフィリ除外コース 8 件を §9 禁止事項 #4 に明文化 (楽天 4 件 + じゃらん 4 件・差分 akane/aburayama 含む)
- 過去事故 4 件を §12 リスクと事故防止に表形式で記録
- 両ディレクトリ同期完了

### ✅ Phase 3.4 完了 (2026-05-11): テンプレ refine

Phase 3.2 で発見された改善点 3 件を `~/blog-template/CLAUDE.md` (v1.1.0) に反映:

1. **§10 をファイル役割分担表に拡張** — リスト 3 項目 → 表形式 (役割 + 更新頻度) + 原則説明 (static / dynamic / cyclic の分類)
2. **§13 プロジェクト構造サマリ新規** — カテゴリ分類 / ペルソナハブ / 特殊分類 (多言語 Tier 等) の 3 サブセクション
3. **§14 Phase 履歴サマリ新規** — 静的保存 (NEXT_SESSION.md は dynamic で直近のみ・本セクションは全件) + 実装例

加えて:
- §12 リスクと事故防止を表形式化
- §11 コミット規約に「コミット粒度」を追加
- README.md v1.0.0 → **v1.1.0** にバージョンアップ・変更履歴記録

成果:
- テンプレ CLAUDE.md: 157 行 → 約 200 行 (改善後)
...
| サブフェーズ | 内容 | 工数 | 着手条件 |
|---|---|---|---|
| **3.3** | 2 つ目のサイト立ち上げ実証 (テンプレから scaffold して動作確認) | 5-8h | 実サイト要望と合わせて実施 |

→ 実サイト立ち上げの具体的なテーマが決まり次第着手。それまでは Phase 3 は **実用段階完了** とみなしてよい。
