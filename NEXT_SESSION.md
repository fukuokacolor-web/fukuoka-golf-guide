...
**最終更新**: 2026-08-12 (✅ 情報系2p + 名門hub 6→10 + コンペhub新設 + 地域ピックアップ会議→宗像/筑紫野パイロット + §7所在地是正 + 日次自動化3本 + 楽天CTA212本 + 多言語方針転換)
**最終 commit**: `3225933` (楽天CTA予約ガイド/レポート) — origin/main へ push 済
**未コミット**: NEXT_SESSION (要 commit)

> ★運用方針 (2026-08-12 決定): **新規ページは JA 単独で作る。EN/KO の新規多言語化はしない。**
> 理由: KR/US 流入ほぼ 0・収益は国内(楽天広域Cookie/じゃらん)主体。既存の EN/KO(124p の content.c-en/c-ko + 132p の hreflang) は **削除しない=据え置き** (消す=JAページ本体編集130ファイル+hreflang除去+404 で SEO マイナスイベント・流入0でも撤去は損)。完全撤去は当面しない。inbound/KO 系施策(Phase 4-B wakamiya KO LP 等)は凍結。

### ✅ 2026-08-08〜12 情報系/ハブ拡充/地域ピックアップ/自動化（commit `a422c9d`→`5dfb2a6`・全push済）

**A. コンテンツ拡充（全て§7の2ソース検証・§9.4 CTA除外厳守・本番push済）**
- **体験記タクソノミー**（`a422c9d`）: `report_taxonomy.json`をSSOT化・report-indexを完全ハブ化
- **情報系2ページ**（`7db7a18`）: `golf-dresscode.html`/`golf-manner.html`（JA単独・ドレスコード/マナー受け皿）。小倉実体験をE-E-A-T引用。guide-indexに「基礎知識」カテゴリ・GSC登録済
- **接待名門ハブ 6→10コース**（`3b0c22a`/`95e83db`）: 門司GC(1934・上田治・宗像でなく門司区)・ザ・クラシックGC(2020&2028日本女子オープン)を名門追加、福岡センチュリー(朝倉)・太宰府GC(1966)を「接待実力派」枠で追加。3言語・ItemList/meta/hero全更新
- **コンペプランハブ新設**（`5f3c83d`）: `hub-compe.html`（じゃらんコンペ特集の福岡10コース・幹事向け）。GSC登録済
- **地域ピックアップ・宗像パイロット**（`5dfb2a6`）: `hub-munakata.html`（宗像市3コース=福岡国際CC/玄海GC(公式のみ)/NEWユーアイGC）。guide-index「エリア・地域で探す」新設・GSC登録済。sitemap計151URL

**B. §7所在地是正**（`6108e31`）: 玄海GC=**宗像市**(hub-businessの「糸島」誤記を3言語是正)/「センチュリー大牟田GC」架空名→「福岡センチュリーGC」(ariake/kurume)

**C. 専門家会議「地域ピックアップ自動化」5名 + fact-checkerペア検証**
- 結論: ①**完全無人公開はNG**→「自動下書き→人間承認→本番」or「文章固定・数値のみ自動」。②「今日のおすすめ」日次AI生成は§12薄い量産/§8 fake freshnessで**却下**→「本日の特価」(検証済価格の数値のみ自動)に再定義。③地域ページは**新規URL乱立せず**、掲載3件以上の市は直接ページ・3件未満は隣接市から補充
- fact-checker確定: 宗像市=3コース(直接ページ成立)/福津市=0件(隣接補充)/古賀市=koga1件/century=**朝倉市**(所在地不明でなかった)/玄海GC=宗像市江口(jalan null・メンバー本人完全予約制)
- **横展開候補(≥3件で直接ページ)**: 糸島(6)・飯塚(6)・北九州(6)・筑紫野(3)。補充型: 福津・古賀・太宰府・宮若

**D. クラウド自動化3本 稼働（全て通知/レポートのみ・本番編集なし）**
- 月次料金ドリフト監査(毎月1日) `trig_011pPPnzSoU1JyQRv7to5ktA`
- 日次 福岡ゴルフニュースdigest(07:30起動) `trig_015NyGgvoNLbyNAUsrATVTVW`
- 日次 じゃらん特価ウォッチ(07:30起動) `trig_01UX2SS5r9NRRTCTzKgTJNBW`
- ②③は**毎朝8:00にGoogleカレンダー(fukuoka.color@gmail.com)へ予定登録+ポップアップ通知**（Gmailコネクタ無しのためカレンダー方式・疎通確認済）

**E. 次の一手 / 要対応**
- **〜2週間後(〜8/26頃)**: GSC(新規5ページのインデックス反映・「宗像 ゴルフ場」等の表示/CTR) + A8収益(じゃらん成約継続) をまとめて効果測定
- **⚠️要確認**: `course-century.html` の**ホール数矛盾**(自ページ27H vs じゃらん20H/18H使用) → 別途是正候補
- **横展開**: 地域ピックアップを糸島/飯塚/北九州へ。`pickup_rules.json`(SSOT・course_data.json読取専用)でルール駆動化→「自動下書き→承認」方式で

---

### ✅ 2026-08-07 効果測定（施策の検証・ポジティブ）
- **実収益(A8)**: 8月7日間で**2件¥1,000発生・全てじゃらんゴルフ**(7月は楽天¥549/月)。7月まで0だった**じゃらん予約が初成約**=リライト/インデックス改善/体験記による流入増が"ゴルフ予約"に変換され始めた([[project_revenue_rakuten_cookie]]更新)。※n=2未確定・月初で断定禁物だが方向性は明確にプラス
- **GSC**: クリック328→372/表示31.7k→36k/順位17.4→17。7/6リライト勢が全て順位改善(芥屋17.1/福岡国際17.0/若松11.3/福岡CC8.6)。hub-business 54→75click。小倉レビュー体験記も8.2位で機能
- **サムネ/インデックス**: 7/29-8/7の対応は再クロール待ちで効果測定は時期尚早(数日〜2週後)
- **⚠️要対応(次の一手)**: `report-kokura`(443表示/18.9位)と`course-kokura`(レビュー追加済)の**小倉カニバリ** → 統合orcanonical整理を検討

### ✅ 2026-08-07 SERPサムネイル全ページ対応 (`74cf0c5`〜`23f639d`)
- GSC「福岡 ゴルフ 名門」3位なのにサムネ無し発見→146中87ページが<img>ゼロ=構造的CTR損
- hero CSS背景→実<img>化を全域展開: course50/hub4/index/LP11/B群15 = 計140ページ<img>化
- デザイン維持(既存背景をimg化)・book-LPは色グラデ半透明化で写真ヒーロー・冪等スクリプト3本
- 残6は404/contact等の機能ページ(SEO対象外・不要)

### ✅ 2026-07-26 GSC分析→専門家会議→会員制トラフィック収益化 (`d7283b0` / `7b1f561`)
- **GSC実データ分析** (3ヶ月): クリック295(加速中)/表示28,700/CTR1%/平均順位17.8(=2ページ目が最大ボトルネック)。hub-business(名門)勝ち・料金クエリ複数1ページ目・**表示の15%(4,284)が予約不可の会員制3コースに流入**(fukuocc順位8.9)
- **専門家会議5名**(SEO/コンテンツ/CVR/IA/行動経済学): 結論=会員制トラフィックの収益化が今日の1手。リライト勢(keya/ukiha/福岡国際)は**8/3まで観測ノータッチ**で合意
- **会員制3コースの代替CTA再設計** (教授:選択設計+CVR案B:楽天直リンク):
  - fukuocc(`d7283b0`)→wakamatsu/genkai(`7b1f561`)の3コース完了
  - 最寄り主役化・獲得フレーム+各代替コースに楽天GORA直リンク併記(2ホップ→1ホップ)・`alternative_cta_direct`計測
  - **§9.4はowner判断(B)で趣旨遵守実装**: 楽天リンクは全て"予約可の別コース"・各ボタンにコース名明示・会員制自身のc_idは無し(誤認防止=読者利益)
- **未着手(会議Tier2)**: クラスタ整合の内部リンク"1本"先行(hub-business→名門系course・IA案)
  - ※小倉レビュー取材案は**オーナー方針で不採用**(取材はしない)。レビュー系クエリの受け皿は取材以外の手段(公式一次情報の厚み増し等)でのみ検討
- **要作業**: GA4_DASHBOARD.md に cta_position=`alternative_cta_direct` の探索レポート定義追加(CVR指摘)・Day14で件数確認

### ✅ 2026-07-06 価格カード第2段 + 午後スルーLP再構築 (`2b6685e` / `f2e8a85`)
- **全46予約可courseを じゃらん照会** → 34件で値取得・12件は取得不能。`price_audit_2026-07.json` に監査証跡を保存
- **価格カード27course更新** (`2b6685e`): 平日/土日を検証値へ(計120)・不明は「予約サイトで確認」へ中立化(42)・
  午後スルー(featured)は全て非数値化(60=裏取り不能かつDecoy=NO)。「2026年7月時点のじゃらん調べ」を3言語で明記
  - **★スコープ厳格化の教訓**: 当初 `VISITOR / WEEKDAY` 等を部分一致で拾い、個別調査済の手作りページ(PLAN 01-03/
    VISITOR·MEMBER/WINTER/SEASON = 16course)の検証値を潰しかけた → **汎用ラベル完全一致のみ**に限定して回避
- **午後スルーLP再構築** (`f2e8a85`): コース側の中立化でLPの「¥3,000台〜」が自己矛盾に → 案2採用。
  トピックは維持し掲載価格を**検証済の平日最安**へ載せ替え+昇順に並べ替え(1位 mission→central ¥5,869)
- 監査データの発見: 浮羽は6月¥4,514→7月¥3,150と**下落**。ズレは高い方にも安い方にも起きる
...
- **じゃらんゴルフ/楽天GORA(ゴルフ予約本命)からの成約は0**。現状マネタイズは「楽天リンク経由の雑多な楽天市場購入」が主体
- ∴収益 ∝ 楽天リンクに触れる訪問者数 → **流入増(SEO/順位)が収益ドライバー**の構造が実データで裏付け
- 専門家会議(SEO/CVR/IA/コンテンツ/インバウンド)結論: **今日の1手=hisayama/dazaifu CTR改善**は不変。but golfwear(楽天リンク)導線が「収益直結」に格上げ。KOは保留継続(KR転換5/122=需要不足)

### ✅ 2026-07-06 hisayama/dazaifu リライト (`0bd687e`)
- title/meta を汎用型→クエリ意図前方一致型 (GSC hisayama366/dazaifu287表示の低CTR取りこぼし対策)
- FAQ各4問 (FAQPage JSON-LD+可視)・じゃらん実勢検証済
- **dazaifu §7是正**: 未確認の午後スルー¥5,500〜/土日¥14,000〜を**全言語(JA/EN/KO)除去**→検証済¥9,800(平日最安)。じゃらん・楽天GORA両方で¥6,000以下・午後スルー確認できず
- hisayama 関連カード: 会員制の福岡CC(和白)→予約可の福岡国際CCへ差替 (死に筋リンク解消)

### ✅ 2026-07-06 golfwear相互リンク+差別化 (`b18b2a5`) / Malbon実写真 (`4bba3b9`)
- golf-wear(Amazon・基礎/マナー) ↔ golfwear-brands(楽天・ブランド/おしゃれ) のカニバリ回避 (統合せず検索意図で棲み分け+相互リンク)
- golfwear-brands の Malbon Featured に**オーナー私物の実写真**(images/malbon-editor.webp)埋込=一次体験E-E-A-T

### ⏭️ 次にやると良いこと (未処理・提案)
- **効果測定**: 2週間後にGSCで hisayama/dazaifu のCTR・順位変化を確認 (title/meta効果)
- **golf-wear.html の§8煽り是正**: 「迷っている間に人気モデルは売り切れます/在庫が一気に」は根拠なき煽り(§8) → 要トーンダウン (今回スコープ外・未着手)
- **四半期の全course料金一括点検**: 料金ズレが常態(浮羽/福岡国際/太宰府で確認) → REWRITE_PLAYBOOK §6 の広域点検を起動
- **楽天Cookie最大化**: 高表示ページに楽天CTAが載っているか点検 (除外4コース以外)

### ✅ 2026-06-22 予約不可コースの死に筋トラフィック対策 (専門家会議承認)
GSC直近7日で過去最高の表示(3,447/7日)・予約不可コースが高表示の発見を受け、5専門家招集:
- ファクトチェック(鈴木+中村 独立): **玄海は休場でなく2023/10再開済・現在会員制**(マッピングJSON訂正済) / 若松・福岡CCは会員制確定
- SEO/CVR/E-E-A-T 全員GO(条件付): 事実を先に明示・煽らない・地理近接の予約可コース・除外非該当
- **実装**: `course-genkai.html` の関連コースを予約不可(福岡カンツリー)→予約可(福岡国際CC/古賀/芥屋)に差替・3言語 (`314666e`)
- **発見**: 若松/福岡カンツリーの関連コースは既に予約可コースへ誘導済 → 差替不要
- **データ訂正**: jalan/rakuten mapping の玄海 long_closure→members_only (`3d17162`・§7)

### ✅ 2026-06-22 hub-business CTR改善 (title/meta)
- 直近7日最多表示(253)・CTR2%の hub-business を最適化 (`8bb096f`)
- title「接待・名門｜…ハブ」→「福岡の名門ゴルフ場 6選｜接待…【2026年版】」(クエリ語順に前方一致)
- **判断**: 高表示course(芥屋/浮羽/フェザント/福岡国際)は title が既にクエリ一致済 → 触らない
  (料金を入れると季節変動で陳腐化§7リスク・低CTRは順位起因でtitleでは解決しない)

### ✅ 2026-06-22 CVR第2段 実装 (予約不可コースの死に筋誘導)
- 若松(142表示・会員制)に「予約できる北九州近隣コース」3件CTA追加 (`ee556bf`)
  - 北九州CC/小倉CC/九州GC八幡・GA4(nav_section:alternative_cta)計測
- 福岡カンツリー(会員制)に「予約できる福岡市近郊コース」3件CTA追加 (`c093a0e`)
  - 西戸崎(同東区)/久山/古賀
- ※玄海は公式/GDOで予約可のため第2段は不要 (related-card差替のみで適切)

### ✅ 2026-06-22 定期リライト運用を仕組み化
- パイロット: 浮羽CC リライト (料金¥4,060→¥4,500鮮度修正・FAQ4問追加・lastmod更新) `b0f9d5c`
  - ファクトチェックで料金が¥450古いと判明 → 定期点検の必要性が実証
- **`REWRITE_PLAYBOOK.md` 策定** (候補選定§2/チェックリスト§3/ガードレール§4/ログ§5/月次フロー§6)
  - CLAUDE.md §10 ファイル役割表に登録済
- **次回リライト候補** (高表示×予約可): pheasant(124) / fukuokakokusai(120) / keya(164)
- 四半期で全courseの料金を じゃらん現行値と一括突合する広域点検を推奨

### ⏭️ 次の伸びしろ (即効性のある打ち手はほぼ出し切り)
- **順位改善が本丸**: 低CTR course は2ページ目(平均20位)起因 → コンテンツ深化・内部/被リンクで順位UP (中長期)
- レビュー/口コミ系(小倉レビュー/久山口コミ): オーナー実地取材が必要 (E-E-A-T・捏造禁止)
- ドレスコード系: 情報ガイドページ化の余地 (但しクエリ量小)
- **効果測定**: 数日〜2週間後にGSCで hub-business CTR / 玄海・若松・福岡CCの alternative_cta 内部遷移 / 新LP順位を確認
...
### ✅ 2026-06-03 1.5ラウンド・27ホールLP作成 (案②)
- 「福岡 ゴルフ場 1.5r」82表示0クリックの受け皿 → `book-fukuoka-27holes.html`
- 検証済の27ホールコース8件 (小郡/筑紫ヶ丘/ムーンレイク/大博多/麻生飯塚/夜須高原/チサン遠賀/クラシック)・全予約可・除外なし
- ハルシネーション回避: 27Hは検証済事実・個別1.5Rプラン料金は捏造せず「予約サイトで確認」誘導
- 各コース詳細へ内部リンク + sitemap 0.8 + 3言語 + GSCインデックス申請済
- **LP 計12本** ・観測非干渉

### ✅ 2026-06-03 目的別予約ガイド ハブ作成 (IA仕上げ)
- `guide-index.html`: 12本のLPを5カテゴリ(料金/プレースタイル/目的シーン/アクセス/言語)に集約
- 各LPへ内部リンク → 全LPの順位底上げ + IA階層の親を付与 (LP乱立を"資産"化)
- CollectionPage + ItemList(12) + BreadcrumbList JSON-LD
- **プレビューQA済**: 12カード・全12リンク200・壊れリンクゼロ・コンソールエラーなし
- sitemap 0.8 + sitemap-guide 3言語先頭 + GSCインデックス申請済
- ⏭️ 残機会 (今は不可): レビュー系(久山口コミ78/太宰府67=実体験取材が必要・捏造禁止) / 予約不可名門の代替誘導(玄海109/若松96=Day42後にcourseページ内部リンク)

### 📌 次セッションの起点 (Day42まで観測継続)
1. **Day 42 (6/17)** `/observation-checkin 42` で Decoy 最終判定 + course ページ凍結解除
2. 凍結解除後の最優先: hisayama(366表示)/dazaifu(287表示) の title/meta CTR改善
3. course凍結解除後: 玄海/若松 等 予約不可コースから予約可代替コースへの内部リンク
4. レビュー系はオーナーの実地取材待ち (E-E-A-T・content-strategist 領域)
5. 新規LPは作り切った (12本+ハブ) → 当面は新規より既存の順位観測・インデックス浸透待ち
...
- `book-fukuoka-beginner.html` 新規作成 (初心者向け予約 LP)
  ...
  - commit: `946bf0e`
- `book-fukuoka-traveler.html` 新規作成 (観光ゴルフ向け予約 LP)
